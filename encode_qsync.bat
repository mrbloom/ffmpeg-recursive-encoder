...
set "sourceFolder=Y:\fast_channels\OTERRA\ENCODE"
...
for /R "%sourceFolder%" %%i in (*.mkv, *.avi, *.mxf) do (
    ...
    set "destFile=%%~dpni_qsync_encoded.mp4"
    ...
    if not exist "!destFile!" (
        ...
        if !fileSize1! equ !fileSize2! (
            ...
            ffmpeg -i "%%i" -vf "format=nv12" -profile:v high -level:v 4.1 -c:v %vCodec% -b:v %bitrate% -map 0:v:0 -filter_complex "[0:a:0][0:a:1]amerge=inputs=2[a]" -map "[a]" -c:a %aCodec% -b:a %audioRate% -ar %sampleRate% -ac 2 "!destFile!"
