...
for /R "%sourceFolder%" %%i in (*.mkv, *.avi, *.mxf, *.mpg) do (
    ...
    set "destFile=%%~dpni_qsync_2pass_encoded.mp4"
    ...
    if not exist "!destFile!" (
        ...
        if !fileSize1! equ !fileSize2! (
            ...
            ffmpeg -i "%%i" -filter_complex "[0:a:0][0:a:1]amerge=inputs=2[a]" -write_tmcd false -map 0:v -c:v %vCodec% -minrate %minrate% -maxrate %maxrate% -bufsize %bufsize%  -b:v %bitrate%  -pass 2 -map "[a]" -c:a %aCodec% -b:a %audioRate% -ar %sampleRate% -ac 2 -passlogfile "!sourceFolder!\!baseName!_ffmpeglog" "!destFile!"	
